...
import test.GrandChild;
import java.lang.reflect.*;
import java.util.*;
import java.io.*;

public class myGrandChild extends GrandChild implements Cloneable {
	public myGrandChild(int MYJHS8Ypy6){super(MYJHS8Ypy6);}

	private ArrayList compareFields = new ArrayList();

	public myGrandChild clone() throws CloneNotSupportedException{
		myGrandChild cloned = (myGrandChild) super.clone();//浅克隆，克隆基本类型的字段
		...
		try {
    		for(Field field: fieldArr){//该类所有的public字段
				...
				//Log.v("clone myGrandChild Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
...
;	}
	...
	@Override public boolean equals(Object other){
        ...
        myGrandChild otherObj = (myGrandChild) other;
        ...
        try{
            for(Field field:getClass().getFields()){
                ...
                //Log.v("compare myGrandChild Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
                //Log.v("compare myGrandChild Field",fieldThisValue+"---"+fieldOtherValue);
