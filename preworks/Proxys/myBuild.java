...
public class myBuild extends Build implements Cloneable {
	public myBuild(){super();}

	private ArrayList compareFields = new ArrayList();

	public myBuild clone() throws CloneNotSupportedException{
		myBuild cloned = (myBuild) super.clone();//浅克隆，克隆基本类型的字段
		...
		try {
    		for(Field field: fieldArr){//该类所有的public字段
				...
				Log.v("clone myBuild Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
	...
	@Override public boolean equals(Object other){
        ...
        myBuild otherObj = (myBuild) other;
        ...
        try{
            for(Field field:getClass().getFields()){
                ...
                Log.v("compare myBuild Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
                Log.v("compare myBuild Field",fieldThisValue+"---"+fieldOtherValue);
