...
public class myHandlerThread extends HandlerThread implements Cloneable {
	public myHandlerThread(java.lang.String GyZFEo30ZI){super(GyZFEo30ZI);}
	public myHandlerThread(java.lang.String StLPvcMNSB, int Gcy3iLWFZI){super(StLPvcMNSB, Gcy3iLWFZI);}

	private ArrayList compareFields = new ArrayList();

	public myHandlerThread clone() throws CloneNotSupportedException{
		myHandlerThread cloned = (myHandlerThread) super.clone();//浅克隆，克隆基本类型的字段
		...
		try {
    		for(Field field: fieldArr){//该类所有的public字段
				...
				Log.v("clone myHandlerThread Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
	...
	@Override public boolean equals(Object other){
        ...
        myHandlerThread otherObj = (myHandlerThread) other;
        ...
        try{
            for(Field field:getClass().getFields()){
                ...
                Log.v("compare myHandlerThread Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
                Log.v("compare myHandlerThread Field",fieldThisValue+"---"+fieldOtherValue);
