...
public class myOutOfMemoryError extends OutOfMemoryError implements Cloneable {
	public myOutOfMemoryError(){super();}
	public myOutOfMemoryError(java.lang.String GYNljnoncx){super(GYNljnoncx);}

	private ArrayList compareFields = new ArrayList();

	public myOutOfMemoryError clone() throws CloneNotSupportedException{
		myOutOfMemoryError cloned = (myOutOfMemoryError) super.clone();//浅克隆，克隆基本类型的字段
		...
		try {
    		for(Field field: fieldArr){//该类所有的public字段
				...
				Log.v("clone myOutOfMemoryError Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
	...
	@Override public boolean equals(Object other){
        ...
        myOutOfMemoryError otherObj = (myOutOfMemoryError) other;
        ...
        try{
            for(Field field:getClass().getFields()){
                ...
                Log.v("compare myOutOfMemoryError Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
                Log.v("compare myOutOfMemoryError Field",fieldThisValue+"---"+fieldOtherValue);
