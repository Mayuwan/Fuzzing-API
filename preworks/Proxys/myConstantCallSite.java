...
public class myConstantCallSite extends ConstantCallSite implements Cloneable {
	public myConstantCallSite(java.lang.invoke.MethodHandle AWSrJqUSti){super(AWSrJqUSti);}
	protected myConstantCallSite(java.lang.invoke.MethodType PpNZb0B7dm, java.lang.invoke.MethodHandle RvCsz7bICB){super(PpNZb0B7dm, RvCsz7bICB);}

	private ArrayList compareFields = new ArrayList();

	public myConstantCallSite clone() throws CloneNotSupportedException{
		myConstantCallSite cloned = (myConstantCallSite) super.clone();//浅克隆，克隆基本类型的字段
		...
		try {
    		for(Field field: fieldArr){//该类所有的public字段
				...
				Log.v("clone myConstantCallSite Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
	...
	@Override public boolean equals(Object other){
        ...
        myConstantCallSite otherObj = (myConstantCallSite) other;
        ...
        try{
            for(Field field:getClass().getFields()){
                ...
                Log.v("compare myConstantCallSite Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
                Log.v("compare myConstantCallSite Field",fieldThisValue+"---"+fieldOtherValue);
