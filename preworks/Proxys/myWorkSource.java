...
public class myWorkSource extends WorkSource implements Cloneable {
	public myWorkSource(){super();}
	public myWorkSource(android.os.WorkSource W1B5wiS7rR){super(W1B5wiS7rR);}

	private ArrayList compareFields = new ArrayList();

	public myWorkSource clone() throws CloneNotSupportedException{
		myWorkSource cloned = (myWorkSource) super.clone();//浅克隆，克隆基本类型的字段
		...
		try {
    		for(Field field: fieldArr){//该类所有的public字段
				...
				Log.v("clone myWorkSource Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
