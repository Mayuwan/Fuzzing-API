...
public class myReferenceQueue extends ReferenceQueue implements Cloneable {
	public myReferenceQueue(){super();}

	private ArrayList compareFields = new ArrayList();

	public myReferenceQueue clone() throws CloneNotSupportedException{
		myReferenceQueue cloned = (myReferenceQueue) super.clone();//浅克隆，克隆基本类型的字段
		...
		try {
    		for(Field field: fieldArr){//该类所有的public字段
				...
				Log.v("clone myReferenceQueue Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
	...
	@Override public boolean equals(Object other){
        ...
        myReferenceQueue otherObj = (myReferenceQueue) other;
        ...
        try{
            for(Field field:getClass().getFields()){
                ...
                Log.v("compare myReferenceQueue Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
                Log.v("compare myReferenceQueue Field",fieldThisValue+"---"+fieldOtherValue);
