...
public class myServiceInfo extends ServiceInfo implements Cloneable {
	public myServiceInfo(){super();}
	public myServiceInfo(android.content.pm.ServiceInfo QP4bmKRjHy){super(QP4bmKRjHy);}

	private ArrayList compareFields = new ArrayList();

	public myServiceInfo clone() throws CloneNotSupportedException{
		myServiceInfo cloned = (myServiceInfo) super.clone();//浅克隆，克隆基本类型的字段
		...
		try {
    		for(Field field: fieldArr){//该类所有的public字段
				...
				Log.v("clone myServiceInfo Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
	...
	@Override public boolean equals(Object other){
        ...
        myServiceInfo otherObj = (myServiceInfo) other;
        ...
        try{
            for(Field field:getClass().getFields()){
                ...
                Log.v("compare myServiceInfo Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
                Log.v("compare myServiceInfo Field",fieldThisValue+"---"+fieldOtherValue);
