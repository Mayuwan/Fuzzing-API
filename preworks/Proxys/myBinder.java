...
public class myBinder extends Binder implements Cloneable {
	public myBinder(){super();}

	private ArrayList compareFields = new ArrayList();

	public myBinder clone() throws CloneNotSupportedException{
		myBinder cloned = (myBinder) super.clone();//浅克隆，克隆基本类型的字段
		...
		try {
    		for(Field field: fieldArr){//该类所有的public字段
				...
				Log.v("clone myBinder Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
	...
	@Override public boolean equals(Object other){
        ...
        myBinder otherObj = (myBinder) other;
        ...
        try{
            for(Field field:getClass().getFields()){
                ...
                Log.v("compare myBinder Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
                Log.v("compare myBinder Field",fieldThisValue+"---"+fieldOtherValue);
