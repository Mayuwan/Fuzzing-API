...
public class myThreadDeath extends ThreadDeath implements Cloneable {
	public myThreadDeath(){super();}

	private ArrayList compareFields = new ArrayList();

	public myThreadDeath clone() throws CloneNotSupportedException{
		myThreadDeath cloned = (myThreadDeath) super.clone();//浅克隆，克隆基本类型的字段
		...
		try {
    		for(Field field: fieldArr){//该类所有的public字段
				...
				Log.v("clone myThreadDeath Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
	...
	@Override public boolean equals(Object other){
        ...
        myThreadDeath otherObj = (myThreadDeath) other;
        ...
        try{
            for(Field field:getClass().getFields()){
                ...
                Log.v("compare myThreadDeath Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
                Log.v("compare myThreadDeath Field",fieldThisValue+"---"+fieldOtherValue);
