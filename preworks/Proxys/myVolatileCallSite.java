...
public class myVolatileCallSite extends VolatileCallSite implements Cloneable {
	public myVolatileCallSite(java.lang.invoke.MethodType KeDPB4B2Di){super(KeDPB4B2Di);}
	public myVolatileCallSite(java.lang.invoke.MethodHandle TZTDSTIb3Y){super(TZTDSTIb3Y);}

	private ArrayList compareFields = new ArrayList();

	public myVolatileCallSite clone() throws CloneNotSupportedException{
		myVolatileCallSite cloned = (myVolatileCallSite) super.clone();//浅克隆，克隆基本类型的字段
		...
		try {
    		for(Field field: fieldArr){//该类所有的public字段
				...
				Log.v("clone myVolatileCallSite Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
	...
	@Override public boolean equals(Object other){
        ...
        myVolatileCallSite otherObj = (myVolatileCallSite) other;
        ...
        try{
            for(Field field:getClass().getFields()){
                ...
                Log.v("compare myVolatileCallSite Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
                Log.v("compare myVolatileCallSite Field",fieldThisValue+"---"+fieldOtherValue);
