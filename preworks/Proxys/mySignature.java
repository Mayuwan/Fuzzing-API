...
public class mySignature extends Signature implements Cloneable {
	public mySignature(byte[] DM5aY6Utgj){super(DM5aY6Utgj);}
	public mySignature(java.lang.String A1EFOqhN86){super(A1EFOqhN86);}

	private ArrayList compareFields = new ArrayList();

	public mySignature clone() throws CloneNotSupportedException{
		mySignature cloned = (mySignature) super.clone();//浅克隆，克隆基本类型的字段
		...
		try {
    		for(Field field: fieldArr){//该类所有的public字段
				...
				Log.v("clone mySignature Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
