...
public class myThreadLocal extends ThreadLocal implements Cloneable {
	public myThreadLocal(){super();}

	private ArrayList compareFields = new ArrayList();

	public myThreadLocal clone() throws CloneNotSupportedException{
		myThreadLocal cloned = (myThreadLocal) super.clone();//浅克隆，克隆基本类型的字段
		...
		try {
    		for(Field field: fieldArr){//该类所有的public字段
				...
				Log.v("clone myThreadLocal Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
	...
	@Override public boolean equals(Object other){
        ...
        myThreadLocal otherObj = (myThreadLocal) other;
        ...
        try{
            for(Field field:getClass().getFields()){
                ...
                Log.v("compare myThreadLocal Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
                Log.v("compare myThreadLocal Field",fieldThisValue+"---"+fieldOtherValue);
