...
public class myApplicationInfo extends ApplicationInfo implements Cloneable {
	public myApplicationInfo(){super();}
	public myApplicationInfo(android.content.pm.ApplicationInfo PIr0YB28ME){super(PIr0YB28ME);}

	private ArrayList compareFields = new ArrayList();

	public myApplicationInfo clone() throws CloneNotSupportedException{
		myApplicationInfo cloned = (myApplicationInfo) super.clone();//浅克隆，克隆基本类型的字段
		...
		try {
    		for(Field field: fieldArr){//该类所有的public字段
				...
				Log.v("clone myApplicationInfo Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
	...
	@Override public boolean equals(Object other){
        ...
        myApplicationInfo otherObj = (myApplicationInfo) other;
        ...
        try{
            for(Field field:getClass().getFields()){
                ...
                Log.v("compare myApplicationInfo Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
                Log.v("compare myApplicationInfo Field",fieldThisValue+"---"+fieldOtherValue);
