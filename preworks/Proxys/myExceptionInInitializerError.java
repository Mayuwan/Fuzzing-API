...
public class myExceptionInInitializerError extends ExceptionInInitializerError implements Cloneable {
	public myExceptionInInitializerError(){super();}
	public myExceptionInInitializerError(java.lang.Throwable QAQmDgy2st){super(QAQmDgy2st);}
	public myExceptionInInitializerError(java.lang.String TlMfaDpeFk){super(TlMfaDpeFk);}

	private ArrayList compareFields = new ArrayList();

	public myExceptionInInitializerError clone() throws CloneNotSupportedException{
		myExceptionInInitializerError cloned = (myExceptionInInitializerError) super.clone();//浅克隆，克隆基本类型的字段
		...
		try {
    		for(Field field: fieldArr){//该类所有的public字段
				...
				Log.v("clone myExceptionInInitializerError Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
	...
	@Override public boolean equals(Object other){
        ...
        myExceptionInInitializerError otherObj = (myExceptionInInitializerError) other;
        ...
        try{
            for(Field field:getClass().getFields()){
                ...
                Log.v("compare myExceptionInInitializerError Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
                Log.v("compare myExceptionInInitializerError Field",fieldThisValue+"---"+fieldOtherValue);
