...
public class myResultReceiver extends ResultReceiver implements Cloneable {
	public myResultReceiver(android.os.Handler YVdQIErUjn){super(YVdQIErUjn);}

	private ArrayList compareFields = new ArrayList();

	public myResultReceiver clone() throws CloneNotSupportedException{
		myResultReceiver cloned = (myResultReceiver) super.clone();//浅克隆，克隆基本类型的字段
		...
		try {
    		for(Field field: fieldArr){//该类所有的public字段
				...
				Log.v("clone myResultReceiver Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
	...
	@Override public boolean equals(Object other){
        ...
        myResultReceiver otherObj = (myResultReceiver) other;
        ...
        try{
            for(Field field:getClass().getFields()){
                ...
                Log.v("compare myResultReceiver Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
                Log.v("compare myResultReceiver Field",fieldThisValue+"---"+fieldOtherValue);
