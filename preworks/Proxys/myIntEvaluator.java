...
public class myIntEvaluator extends IntEvaluator implements Cloneable {
	public myIntEvaluator(){super();}

	private ArrayList compareFields = new ArrayList();

	public myIntEvaluator clone() throws CloneNotSupportedException{
		myIntEvaluator cloned = (myIntEvaluator) super.clone();//浅克隆，克隆基本类型的字段
		...
		try {
    		for(Field field: fieldArr){//该类所有的public字段
				...
				Log.v("clone myIntEvaluator Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
	...
	@Override public boolean equals(Object other){
        ...
        myIntEvaluator otherObj = (myIntEvaluator) other;
        ...
        try{
            for(Field field:getClass().getFields()){
                ...
                Log.v("compare myIntEvaluator Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
                Log.v("compare myIntEvaluator Field",fieldThisValue+"---"+fieldOtherValue);
