...
public class myRectEvaluator extends RectEvaluator implements Cloneable {
	public myRectEvaluator(){super();}
	public myRectEvaluator(android.graphics.Rect YGSqnAijXQ){super(YGSqnAijXQ);}

	private ArrayList compareFields = new ArrayList();

	public myRectEvaluator clone() throws CloneNotSupportedException{
		myRectEvaluator cloned = (myRectEvaluator) super.clone();//浅克隆，克隆基本类型的字段
		...
		try {
    		for(Field field: fieldArr){//该类所有的public字段
				...
				Log.v("clone myRectEvaluator Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
	...
	@Override public boolean equals(Object other){
        ...
        myRectEvaluator otherObj = (myRectEvaluator) other;
        ...
        try{
            for(Field field:getClass().getFields()){
                ...
                Log.v("compare myRectEvaluator Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
                Log.v("compare myRectEvaluator Field",fieldThisValue+"---"+fieldOtherValue);
