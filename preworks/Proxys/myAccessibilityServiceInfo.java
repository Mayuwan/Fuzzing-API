...
public class myAccessibilityServiceInfo extends AccessibilityServiceInfo implements Cloneable {
	public myAccessibilityServiceInfo(){super();}

	private ArrayList compareFields = new ArrayList();

	public myAccessibilityServiceInfo clone() throws CloneNotSupportedException{
		myAccessibilityServiceInfo cloned = (myAccessibilityServiceInfo) super.clone();//浅克隆，克隆基本类型的字段
		...
		try {
    		for(Field field: fieldArr){//该类所有的public字段
				...
				Log.v("clone myAccessibilityServiceInfo Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
