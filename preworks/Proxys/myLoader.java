...
public class myLoader extends Loader implements Cloneable {
	public myLoader(android.content.Context DX0K5Lsq1y){super(DX0K5Lsq1y);}

	private ArrayList compareFields = new ArrayList();

	public myLoader clone() throws CloneNotSupportedException{
		myLoader cloned = (myLoader) super.clone();//浅克隆，克隆基本类型的字段
		...
		try {
    		for(Field field: fieldArr){//该类所有的public字段
				...
				Log.v("clone myLoader Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
	...
	@Override public boolean equals(Object other){
        ...
        myLoader otherObj = (myLoader) other;
        ...
        try{
            for(Field field:getClass().getFields()){
                ...
                Log.v("compare myLoader Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
                Log.v("compare myLoader Field",fieldThisValue+"---"+fieldOtherValue);
