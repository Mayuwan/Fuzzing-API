...
public class myUriMatcher extends UriMatcher implements Cloneable {
	public myUriMatcher(int ZCJeJUHvFj){super(ZCJeJUHvFj);}

	private ArrayList compareFields = new ArrayList();

	public myUriMatcher clone() throws CloneNotSupportedException{
		myUriMatcher cloned = (myUriMatcher) super.clone();//浅克隆，克隆基本类型的字段
		...
		try {
    		for(Field field: fieldArr){//该类所有的public字段
				...
				Log.v("clone myUriMatcher Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
	...
	@Override public boolean equals(Object other){
        ...
        myUriMatcher otherObj = (myUriMatcher) other;
        ...
        try{
            for(Field field:getClass().getFields()){
                ...
                Log.v("compare myUriMatcher Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
                Log.v("compare myUriMatcher Field",fieldThisValue+"---"+fieldOtherValue);
