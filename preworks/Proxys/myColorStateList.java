...
public class myColorStateList extends ColorStateList implements Cloneable {
	public myColorStateList(int[][] QgSwRoombb, int[] SLQXFzwt0P){super(QgSwRoombb, SLQXFzwt0P);}

	private ArrayList compareFields = new ArrayList();

	public myColorStateList clone() throws CloneNotSupportedException{
		myColorStateList cloned = (myColorStateList) super.clone();//浅克隆，克隆基本类型的字段
		...
		try {
    		for(Field field: fieldArr){//该类所有的public字段
				...
				Log.v("clone myColorStateList Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
	...
	@Override public boolean equals(Object other){
        ...
        myColorStateList otherObj = (myColorStateList) other;
        ...
        try{
            for(Field field:getClass().getFields()){
                ...
                Log.v("compare myColorStateList Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
                Log.v("compare myColorStateList Field",fieldThisValue+"---"+fieldOtherValue);
