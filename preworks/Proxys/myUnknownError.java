...
public class myUnknownError extends UnknownError implements Cloneable {
	public myUnknownError(){super();}
	public myUnknownError(java.lang.String BEpyctrjqY){super(BEpyctrjqY);}

	private ArrayList compareFields = new ArrayList();

	public myUnknownError clone() throws CloneNotSupportedException{
		myUnknownError cloned = (myUnknownError) super.clone();//浅克隆，克隆基本类型的字段
		...
		try {
    		for(Field field: fieldArr){//该类所有的public字段
				...
				Log.v("clone myUnknownError Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
	...
	@Override public boolean equals(Object other){
        ...
        myUnknownError otherObj = (myUnknownError) other;
        ...
        try{
            for(Field field:getClass().getFields()){
                ...
                Log.v("compare myUnknownError Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
                Log.v("compare myUnknownError Field",fieldThisValue+"---"+fieldOtherValue);
