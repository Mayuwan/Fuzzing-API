...
public class myContextWrapper extends ContextWrapper implements Cloneable {
	public myContextWrapper(android.content.Context FL22NOtTJS){super(FL22NOtTJS);}

	private ArrayList compareFields = new ArrayList();

	public myContextWrapper clone() throws CloneNotSupportedException{
		myContextWrapper cloned = (myContextWrapper) super.clone();//浅克隆，克隆基本类型的字段
		...
		try {
    		for(Field field: fieldArr){//该类所有的public字段
				...
				Log.v("clone myContextWrapper Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
	...
	@Override public boolean equals(Object other){
        ...
        myContextWrapper otherObj = (myContextWrapper) other;
        ...
        try{
            for(Field field:getClass().getFields()){
                ...
                Log.v("compare myContextWrapper Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
                Log.v("compare myContextWrapper Field",fieldThisValue+"---"+fieldOtherValue);
