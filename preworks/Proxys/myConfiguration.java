...
public class myConfiguration extends Configuration implements Cloneable {
	public myConfiguration(){super();}
	public myConfiguration(android.content.res.Configuration KsiS0wFCLa){super(KsiS0wFCLa);}

	private ArrayList compareFields = new ArrayList();

	public myConfiguration clone() throws CloneNotSupportedException{
		myConfiguration cloned = (myConfiguration) super.clone();//浅克隆，克隆基本类型的字段
		...
		try {
    		for(Field field: fieldArr){//该类所有的public字段
				...
				Log.v("clone myConfiguration Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
