...
public class myVerifyError extends VerifyError implements Cloneable {
	public myVerifyError(){super();}
	public myVerifyError(java.lang.String RPm1fpM0ED){super(RPm1fpM0ED);}

	private ArrayList compareFields = new ArrayList();

	public myVerifyError clone() throws CloneNotSupportedException{
		myVerifyError cloned = (myVerifyError) super.clone();//浅克隆，克隆基本类型的字段
		...
		try {
    		for(Field field: fieldArr){//该类所有的public字段
				...
				Log.v("clone myVerifyError Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
	...
	@Override public boolean equals(Object other){
        ...
        myVerifyError otherObj = (myVerifyError) other;
        ...
        try{
            for(Field field:getClass().getFields()){
                ...
                Log.v("compare myVerifyError Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
                Log.v("compare myVerifyError Field",fieldThisValue+"---"+fieldOtherValue);
