...
public class myClassFormatError extends ClassFormatError implements Cloneable {
	public myClassFormatError(){super();}
	public myClassFormatError(java.lang.String Lwk7bnD6Vq){super(Lwk7bnD6Vq);}

	private ArrayList compareFields = new ArrayList();

	public myClassFormatError clone() throws CloneNotSupportedException{
		myClassFormatError cloned = (myClassFormatError) super.clone();//浅克隆，克隆基本类型的字段
		...
		try {
    		for(Field field: fieldArr){//该类所有的public字段
				...
				Log.v("clone myClassFormatError Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
	...
	@Override public boolean equals(Object other){
        ...
        myClassFormatError otherObj = (myClassFormatError) other;
        ...
        try{
            for(Field field:getClass().getFields()){
                ...
                Log.v("compare myClassFormatError Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
                Log.v("compare myClassFormatError Field",fieldThisValue+"---"+fieldOtherValue);
