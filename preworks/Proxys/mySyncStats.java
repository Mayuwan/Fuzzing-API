...
public class mySyncStats extends SyncStats implements Cloneable {
	public mySyncStats(){super();}
	public mySyncStats(android.os.Parcel O8tdWsWowJ){super(O8tdWsWowJ);}

	private ArrayList compareFields = new ArrayList();

	public mySyncStats clone() throws CloneNotSupportedException{
		mySyncStats cloned = (mySyncStats) super.clone();//浅克隆，克隆基本类型的字段
		...
		try {
    		for(Field field: fieldArr){//该类所有的public字段
				...
				Log.v("clone mySyncStats Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
	...
	@Override public boolean equals(Object other){
        ...
        mySyncStats otherObj = (mySyncStats) other;
        ...
        try{
            for(Field field:getClass().getFields()){
                ...
                Log.v("compare mySyncStats Field",Modifier.toString(field.getModifiers())+" "+field.getType().getName()+" "+field.getName());
                Log.v("compare mySyncStats Field",fieldThisValue+"---"+fieldOtherValue);
